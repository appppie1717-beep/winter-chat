...
# =====================================================================
# 🎨 [디자인 정밀 광택 2.4.8] 라이트 모드 렌더링 완벽 수정 & UI 최적화
# =====================================================================
...
if st.session_state.theme == "light":
    ...
    /* 👇 사진에서 까맣게 떴던 하단 채팅 입력창 구역 하얗게 덮어쓰기 */
    ...
    [data-testid="stChatInput"] textarea { color: #1E1E1E !important; }
...
# =====================================================================
    ...
    with tab2:
        ...
        with st.container(height=500):
            st.markdown("""
            **[ v2.4.8 ] 2026.03.31 (화)**
            * **[20:10] 📱 UI/UX 최적화:** 라이트 모드 시 채팅창 하단이 까맣게 출력되던 버그를 완벽 수정했습니다.
...
# =====================================================================
    ...
    if user_input := st.chat_input("겨울이에게 메시지 보내기"):
        ...
        try:
            ...
            st.error("앗! 서버가 잠깐 숨을 고르고 있어요. 다시 메시지를 보내주세요! 🚨")
            st.stop()
        ...
        if st.session_state.turn_count >= 10: 
            with st.spinner("❄️ 겨울이가 당신과의 기억을 정리하고 있습니다..."):
                try:
                    ...
                    supabase.table("chat_memory").delete().eq("username", user_name).eq("role", "core_memory").execute()
